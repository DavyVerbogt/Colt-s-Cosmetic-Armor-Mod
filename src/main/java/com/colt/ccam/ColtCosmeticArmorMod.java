...
public class ColtCosmeticArmorMod {
    ...
    public void enqueueIMC(final InterModEnqueueEvent event) {
        ...
        InterModComms.sendTo("curios", SlotTypeMessage.REGISTER_TYPE, () -> new SlotTypeMessage.Builder("feet")
                .priority(220).icon(PlayerContainer.EMPTY_ARMOR_SLOT_BOOTS).cosmetic().build());
        InterModComms.sendTo("curios", SlotTypeMessage.REGISTER_TYPE,
                () -> new SlotTypeMessage.Builder("tail").priority(90)
                        .icon(new ResourceLocation(ColtCosmeticArmorMod.MOD_ID, "item/tail_slot")).cosmetic()
                        .build());
    }
    ...
    public static class ClientProxy {
        ...
        @SubscribeEvent
        public static void CreateIcons(TextureStitchEvent.Pre evt) {

            if (evt.getMap().getTextureLocation() == PlayerContainer.LOCATION_BLOCKS_TEXTURE) {

                evt.addSprite(new ResourceLocation(ColtCosmeticArmorMod.MOD_ID, "item/tail_slot"));
            }
        }
    }
    /*
